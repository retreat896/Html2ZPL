...
export default function Sidebar({ isOpen, toggleSidebar }) {
  const { project, activeLabelId, setActiveLabelId, addLabel, deleteLabel, renameLabel } = useProject();
                      ...
                      <div className={clsx('overflow-hidden transition-all duration-300 ease-in-out', isLabelsOpen && isOpen ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0')}>
                          {isOpen && (
                              <div className="space-y-1 mt-1">
                                  ...
                                  <button onClick={addLabel} className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-gray-50 dark:hover:bg-gray-800 rounded-lg transition-all duration-200">
